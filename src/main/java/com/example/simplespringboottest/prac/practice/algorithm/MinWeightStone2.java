 ...
 */
public class MinWeightStone2 {
    public static void main(String[] args) {
        int []arr = {31,26,33,21,40};
        getMinWeightStone(arr);
    }

    private static int getMinWeightStone(int[] arr) {
        int value =0;
        for (int i = 0; i < arr.length-1; i++) {
            for (int j = i+1; j < arr.length-i-1; j++) {
                value = Math.abs(value) - Math.abs(arr[i]-arr[j]);
            }
        }
